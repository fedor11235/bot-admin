    ...
    >
      ...
      <template v-slot:body-cell-actions="props">
        <q-td class="actions" :props="props">
          ...
          <q-btn size="sm" rounded unelevated color="purple" @click="handlerSendDateBot(props.row.id)" label="отправить список дат"></q-btn>
        </q-td>       
      </template>
    </q-table>
...
async function handlerSendDateBot(id: any) {
  const rec = recommendations.value.find((rec: any) => rec.id === id)
  const text = `${(rec as any).username} ${(rec as any).data_list}`
  sendAdminText(text)
}
